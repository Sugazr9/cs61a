...
def cumulative_sum(t):
    ...
    if t.is_leaf():
        ...
        branches = [cumulative_sum(children) for children in t.children]
        return Tree(t.label + sum([children.label for children in branches]), branches)
...
def has_cycle(link):
    ...
    while True:
        ...
        if check_link.rest is link:
            return True
        if check_link.rest is Link.empty:
            return False
...
def is_valid_bst(bst):
    ...
    if bst is BinTree.empty:
        return True
    elif bst_max(bst) == bst.label and bst_min(bst) == bst.label:
        return True
    elif bst_max(bst) < bst.label or bst.label < bst_min(bst):
        return False
    if is_valid_bst(bst.left) and is_valid_bst(bst.right):
        return True
    return False
