...
def mergesort(seq):
    ...
    return merge(mergesort(seq[:spot]), mergesort(seq[spot:]))
...
def balanced(m):
    ...
    for x in sides(m):
        if not is_weight(end(x)):
            return balanced(x)
        if not total_weight(end(sides(m)[0]))*length(sides(m)[0]) == total_weight(end(sides(m)[1]))*length(sides(m)[1]):
            return False
    return True
...
def simplify(expr):
    ...
    left_operand = simplify(left_opnd(expr))
    right_operand = simplify(right_opnd(expr))
    
    if oper(expr) == '-':
        if same_expr(left_operand, right_operand):
            return ZERO
    if oper(expr) == '*':
        if left_operand == ZERO or right_operand == ZERO:
            return ZERO
        if left_operand == ONE:
            return right_operand
        if right_operand == ONE:
            return left_operand
    if oper(expr) == '+' or oper(expr) == '-':
        if left_operand == ZERO:
            return right_operand
        if right_operand == ZERO:
            return left_operand

    return tree(oper(expr), [left_operand, right_operand])
