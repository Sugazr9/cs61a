...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN Question 10
        while vals is not nil and formals is not nil:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        def helper(func, args):
            return func(*args)
        try:
            return helper(self.fn, python_args)
        except TypeError:
            raise SchemeError(TypeError)
        # END Question 4
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        """Make a frame that binds the formal parameters to ARGS, a
...
def do_if_form(expressions, env):
    ...
    results = expressions.second
    if scheme_true(cond):
        return scheme_eval(results.first, env, True)
    else:
        if results.second is nil:
            return
        return scheme_eval(results.second.first, env, True)
    # END Question 13
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        cond = scheme_eval(clause.first, env)
        if scheme_true(cond):
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        values = Pair(scheme_eval(current.second.first, env), values)
...
def check_formals(formals):
    ...
    while current_formal:
        current_val = current_formal.first
        if not scheme_symbolp(current_val):
            raise SchemeError(SyntaxError)
        if current_val in variables:
            raise SchemeError(SyntaxError)
        variables += [current_formal.first]
        current_formal = current_formal.second
    # END Question 11B
...
def complete_eval(val):
    ...
    expression part. Otherwise, simply returns VAL."""
