...
(define (length lst)
  (define (length+ lst total)
    (if (null? lst) total (length+ (cdr lst) (+ total 1))))
  (length+ lst 0)
)
(length '(3 4 5 3 55))
; expect 5

(define (filter-list f lst)
  (cond ((null? lst) nil)
...
(filter-list odd? '(3 4 7 9 32))

; expect (3 7 9)

(define (multitask x y)
  (+ x 3) (* 2 4) (* 3 y) (list x y))
